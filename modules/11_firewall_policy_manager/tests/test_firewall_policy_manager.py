...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.11_firewall_policy_manager.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_firewall_policy_manager_lifecycle():
    async def _run():
...
def test_firewall_policy_manager_engine_functionality():
    Module = get_module_class()
    mod = Module()
