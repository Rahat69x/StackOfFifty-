...
## Description
Orchestrates defensive firewall policies, evaluates ACL rules, and detects shadow conflicts
...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/firewall_policy_manager/status`).
...
│   └── firewall_policy_manager.py       # Core engine and domain logic
├── tests/
│   └── test_firewall_policy_manager.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/firewall_policy_manager.git
   cd firewall_policy_manager
   ```
...
- `GET /firewall_policy_manager/status`: Current execution metrics and telemetry summary
