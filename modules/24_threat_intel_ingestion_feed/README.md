...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/threat_intel_ingestion_feed/status`).
...
│   └── threat_intel_ingestion_feed.py       # Core engine and domain logic
├── tests/
│   └── test_threat_intel_ingestion_feed.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/threat_intel_ingestion_feed.git
   cd threat_intel_ingestion_feed
   ```
...
- `GET /threat_intel_ingestion_feed/status`: Current execution metrics and telemetry summary
