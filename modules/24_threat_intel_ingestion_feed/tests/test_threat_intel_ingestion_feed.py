...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.24_threat_intel_ingestion_feed.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_threat_intel_ingestion_feed_lifecycle():
    async def _run():
...
def test_threat_intel_ingestion_feed_engine_functionality():
    Module = get_module_class()
    mod = Module()
