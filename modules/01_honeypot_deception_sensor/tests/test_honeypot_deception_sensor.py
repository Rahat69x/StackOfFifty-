...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.01_honeypot_deception_sensor.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_honeypot_deception_sensor_lifecycle():
    async def _run():
...
def test_honeypot_deception_sensor_engine_functionality():
    Module = get_module_class()
    mod = Module()
