...
## Description
Deploys non-intrusive decoy sensors to attract, log, and analyze unauthorized connection telemetry in a protected lab environment
...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/honeypot_deception_sensor/status`).
...
│   └── honeypot_deception_sensor.py       # Core engine and domain logic
├── tests/
│   └── test_honeypot_deception_sensor.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/honeypot_deception_sensor.git
   cd honeypot_deception_sensor
   ```
...
- `GET /honeypot_deception_sensor/status`: Current execution metrics and telemetry summary
