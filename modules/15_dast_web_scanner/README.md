...
## Description
Performs passive dynamic application security audits for OWASP Top 10 indicators
...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/dast_web_scanner/status`).
...
│   └── dast_web_scanner.py       # Core engine and domain logic
├── tests/
│   └── test_dast_web_scanner.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/dast_web_scanner.git
   cd dast_web_scanner
   ```
...
- `GET /dast_web_scanner/status`: Current execution metrics and telemetry summary
