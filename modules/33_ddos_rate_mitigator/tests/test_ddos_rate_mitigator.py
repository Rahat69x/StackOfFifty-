...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.33_ddos_rate_mitigator.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_ddos_rate_mitigator_lifecycle():
    async def _run():
...
def test_ddos_rate_mitigator_engine_functionality():
    Module = get_module_class()
    mod = Module()
