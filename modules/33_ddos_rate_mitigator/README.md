# DDoS Rate Mitigator & Flood Detector
...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/ddos_rate_mitigator/status`).
...
│   └── ddos_rate_mitigator.py       # Core engine and domain logic
├── tests/
│   └── test_ddos_rate_mitigator.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/ddos_rate_mitigator.git
   cd ddos_rate_mitigator
   ```
...
- `GET /ddos_rate_mitigator/status`: Current execution metrics and telemetry summary
