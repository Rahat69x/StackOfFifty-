...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.31_rootkit_hunter_scanner.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_rootkit_hunter_scanner_lifecycle():
    async def _run():
...
def test_rootkit_hunter_scanner_engine_functionality():
    Module = get_module_class()
    mod = Module()
