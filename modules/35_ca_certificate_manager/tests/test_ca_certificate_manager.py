...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.35_ca_certificate_manager.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_ca_certificate_manager_lifecycle():
    async def _run():
...
def test_ca_certificate_manager_engine_functionality():
    Module = get_module_class()
    mod = Module()
