...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.06_lab_environment_orchestrator.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_lab_environment_orchestrator_lifecycle():
    async def _run():
...
def test_lab_environment_orchestrator_engine_functionality():
    Module = get_module_class()
    mod = Module()
