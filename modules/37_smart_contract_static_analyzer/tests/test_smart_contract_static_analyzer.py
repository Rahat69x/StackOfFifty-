...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.37_smart_contract_static_analyzer.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_smart_contract_static_analyzer_lifecycle():
    async def _run():
...
def test_smart_contract_static_analyzer_engine_functionality():
    Module = get_module_class()
    mod = Module()
