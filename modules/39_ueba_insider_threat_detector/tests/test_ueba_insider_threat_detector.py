...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.39_ueba_insider_threat_detector.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_ueba_insider_threat_detector_lifecycle():
    async def _run():
...
def test_ueba_insider_threat_detector_engine_functionality():
    Module = get_module_class()
    mod = Module()
