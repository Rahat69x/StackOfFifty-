...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.13_secure_web_bastion.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_secure_web_bastion_lifecycle():
    async def _run():
...
def test_secure_web_bastion_engine_functionality():
    Module = get_module_class()
    mod = Module()
