...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.28_ml_network_ids_engine.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_ml_network_ids_engine_lifecycle():
    async def _run():
...
def test_ml_network_ids_engine_engine_functionality():
    Module = get_module_class()
    mod = Module()
