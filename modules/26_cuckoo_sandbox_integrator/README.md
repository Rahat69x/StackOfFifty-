...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/cuckoo_sandbox_integrator/status`).
...
│   └── cuckoo_sandbox_integrator.py       # Core engine and domain logic
├── tests/
│   └── test_cuckoo_sandbox_integrator.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/cuckoo_sandbox_integrator.git
   cd cuckoo_sandbox_integrator
   ```
...
- `GET /cuckoo_sandbox_integrator/status`: Current execution metrics and telemetry summary
