...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.42_apt_threat_actor_matcher.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_apt_threat_actor_matcher_lifecycle():
    async def _run():
...
def test_apt_threat_actor_matcher_engine_functionality():
    Module = get_module_class()
    mod = Module()
