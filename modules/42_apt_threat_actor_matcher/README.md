# APT Threat Actor Campaign Matcher
...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/apt_threat_actor_matcher/status`).
...
│   └── apt_threat_actor_matcher.py       # Core engine and domain logic
├── tests/
│   └── test_apt_threat_actor_matcher.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/apt_threat_actor_matcher.git
   cd apt_threat_actor_matcher
   ```
...
- `GET /apt_threat_actor_matcher/status`: Current execution metrics and telemetry summary
