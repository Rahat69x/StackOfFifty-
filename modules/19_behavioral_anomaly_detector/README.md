...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/behavioral_anomaly_detector/status`).
...
│   └── behavioral_anomaly_detector.py       # Core engine and domain logic
├── tests/
│   └── test_behavioral_anomaly_detector.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/behavioral_anomaly_detector.git
   cd behavioral_anomaly_detector
   ```
...
- `GET /behavioral_anomaly_detector/status`: Current execution metrics and telemetry summary
