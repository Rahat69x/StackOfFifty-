...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.19_behavioral_anomaly_detector.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_behavioral_anomaly_detector_lifecycle():
    async def _run():
...
def test_behavioral_anomaly_detector_engine_functionality():
    Module = get_module_class()
    mod = Module()
