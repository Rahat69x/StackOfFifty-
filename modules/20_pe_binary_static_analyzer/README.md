...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/pe_binary_static_analyzer/status`).
...
│   └── pe_binary_static_analyzer.py       # Core engine and domain logic
├── tests/
│   └── test_pe_binary_static_analyzer.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/pe_binary_static_analyzer.git
   cd pe_binary_static_analyzer
   ```
...
- `GET /pe_binary_static_analyzer/status`: Current execution metrics and telemetry summary
