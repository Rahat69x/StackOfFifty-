...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.05_digital_forensics_toolkit.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_digital_forensics_toolkit_lifecycle():
    async def _run():
...
def test_digital_forensics_toolkit_engine_functionality():
    Module = get_module_class()
    mod = Module()
