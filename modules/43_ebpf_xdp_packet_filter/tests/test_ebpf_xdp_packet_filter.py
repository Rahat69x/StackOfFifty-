...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.43_ebpf_xdp_packet_filter.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_ebpf_xdp_packet_filter_lifecycle():
    async def _run():
...
def test_ebpf_xdp_packet_filter_engine_functionality():
    Module = get_module_class()
    mod = Module()
