...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.36_cve_mitre_vuln_correlator.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_cve_mitre_vuln_correlator_lifecycle():
    async def _run():
...
def test_cve_mitre_vuln_correlator_engine_functionality():
    Module = get_module_class()
    mod = Module()
