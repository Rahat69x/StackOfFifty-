...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.23_secure_encrypted_tunnel.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_secure_encrypted_tunnel_lifecycle():
    async def _run():
...
def test_secure_encrypted_tunnel_engine_functionality():
    Module = get_module_class()
    mod = Module()
