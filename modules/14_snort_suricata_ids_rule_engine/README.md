# Snort / Suricata IDS Rule Engine
...
- **FastAPI REST API**: Native OpenAPI-compliant endpoints (`/snort_suricata_ids_rule_engine/status`).
...
│   └── snort_suricata_ids_rule_engine.py       # Core engine and domain logic
├── tests/
│   └── test_snort_suricata_ids_rule_engine.py     # Automated lifecycle & engine unit tests
...
1. Clone the repository:
   ```bash
   git clone https://github.com/<your-username>/snort_suricata_ids_rule_engine.git
   cd snort_suricata_ids_rule_engine
   ```
...
- `GET /snort_suricata_ids_rule_engine/status`: Current execution metrics and telemetry summary
