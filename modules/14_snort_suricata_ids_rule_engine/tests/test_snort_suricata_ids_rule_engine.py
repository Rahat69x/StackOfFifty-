...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.14_snort_suricata_ids_rule_engine.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_snort_suricata_ids_rule_engine_lifecycle():
    async def _run():
...
def test_snort_suricata_ids_rule_engine_engine_functionality():
    Module = get_module_class()
    mod = Module()
