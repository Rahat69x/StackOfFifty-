...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.48_kernel_module_integrity_guard.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_kernel_module_integrity_guard_lifecycle():
    async def _run():
...
def test_kernel_module_integrity_guard_engine_functionality():
    Module = get_module_class()
    mod = Module()
