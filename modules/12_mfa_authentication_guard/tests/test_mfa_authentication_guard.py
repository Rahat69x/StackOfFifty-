...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.12_mfa_authentication_guard.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_mfa_authentication_guard_lifecycle():
    async def _run():
...
def test_mfa_authentication_guard_engine_functionality():
    Module = get_module_class()
    mod = Module()
