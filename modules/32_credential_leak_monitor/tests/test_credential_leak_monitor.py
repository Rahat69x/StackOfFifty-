...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.32_credential_leak_monitor.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_credential_leak_monitor_lifecycle():
    async def _run():
...
def test_credential_leak_monitor_engine_functionality():
    Module = get_module_class()
    mod = Module()
