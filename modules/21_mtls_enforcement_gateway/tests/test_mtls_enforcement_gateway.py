...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.21_mtls_enforcement_gateway.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_mtls_enforcement_gateway_lifecycle():
    async def _run():
...
def test_mtls_enforcement_gateway_engine_functionality():
    Module = get_module_class()
    mod = Module()
