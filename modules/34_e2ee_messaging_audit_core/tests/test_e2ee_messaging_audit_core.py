...
def get_module_class():
    try:
        import importlib
        mod_pkg = importlib.import_module("modules.34_e2ee_messaging_audit_core.main")
        return getattr(mod_pkg, "Module")
    except (ImportError, ModuleNotFoundError):
        import main
        return getattr(main, "Module")

def test_e2ee_messaging_audit_core_lifecycle():
    async def _run():
...
def test_e2ee_messaging_audit_core_engine_functionality():
    Module = get_module_class()
    mod = Module()
